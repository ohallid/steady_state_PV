function [mm] = M2p_Zn(bm, HL_bar, c, A, x, z, T, f, Ht_bar, sigma, N1)
%
% For steady p field 
%
...
% x-variation
alpha_n      = f     * T * bm * sigma         * ( c * HL_bar ) / ( sigma * sigma * f * f - c * HL_bar * c * HL_bar);
beta_n       = f * f * T * bm * sigma * sigma                  / ( sigma * sigma * f * f - c * HL_bar * c * HL_bar );
x_var        = - alpha_n * exp ( - f * abs(x) / ( c * HL_bar ) ) ...
